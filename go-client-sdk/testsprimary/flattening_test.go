...
func TestRequiredBodyAllOptional(t *testing.T) {
	...
	req := shared.ObjWithOptionalProperties{
		OptStr: sdk.String("body test"),
		OptInt: sdk.Int64(1),
	}
	res, err := s.Flattening.RequiredBodyAllOptional(context.Background(), &req)
	require.NoError(t, err)
	require.NotNil(t, res)
	assert.Equal(t, http.StatusOK, res.HTTPMeta.Response.StatusCode)
	assert.NotNil(t, res.Res.JSON)
	assert.Equal(t, sdk.String("body test"), res.Res.JSON.OptStr)
	assert.Equal(t, sdk.Int64(1), res.Res.JSON.OptInt)

	res, err = s.Flattening.RequiredBodyAllOptional(context.Background(), nil)
	require.NoError(t, err)
	require.NotNil(t, res)
	assert.Equal(t, http.StatusOK, res.HTTPMeta.Response.StatusCode)
	assert.NotNil(t, res.Res.JSON)
	assert.Nil(t, res.Res.JSON.OptStr)
	assert.Nil(t, res.Res.JSON.OptInt)
}
