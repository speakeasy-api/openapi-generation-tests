...
func TestRequestBodyPutMultipartFileStreaming(t *testing.T) {
	recordTest("request-bodies-put-multipart-file-streaming")
	...
	file, err := os.Open("testdata/testUpload.json")
	require.NoError(t, err)
...
func TestRequestBodyPutBytesStreaming(t *testing.T) {
	...
	res, err := s.RequestBodies.RequestBodyPutBytes(context.Background(), file)
	require.NoError(t, err)
	require.NotNil(t, res)
	assert.Equal(t, http.StatusOK, res.HTTPMeta.Response.StatusCode)
	assert.NotNil(t, res.Res)
