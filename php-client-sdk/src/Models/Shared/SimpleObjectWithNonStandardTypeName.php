<?php

/**
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

declare(strict_types=1);

namespace OpenAPI\OpenAPI\Models\Shared;

use Brick\DateTime\LocalDate;
/**
 * SimpleObjectWithNonStandardTypeName - A simple object that uses all our supported primitive types and enums and has optional properties.
 *
 * https://speakeasy.com/docs - A link to the external docs.
 *
 * @see https://speakeasy.com/docs
 */
class SimpleObjectWithNonStandardTypeName
{
    /**
     * An any property.
     *
     * @var mixed $any
     */
    #[\JMS\Serializer\Annotation\SerializedName('any')]
    #[\JMS\Serializer\Annotation\Type('mixed')]
    public mixed $any;

    /**
     *
     * @var ?int $bigint
     */
    #[\JMS\Serializer\Annotation\SerializedName('bigint')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?int $bigint = null;

    /**
     *
     * @var ?string $bigintStr
     */
    #[\JMS\Serializer\Annotation\SerializedName('bigintStr')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?string $bigintStr = null;

    /**
     * A boolean property.
     *
     * @var bool $bool
     */
    #[\JMS\Serializer\Annotation\SerializedName('bool')]
    public bool $bool;

    /**
     * An optional boolean property.
     *
     * @var ?bool $boolOpt
     */
    #[\JMS\Serializer\Annotation\SerializedName('boolOpt')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?bool $boolOpt = null;

    /**
     * A date property.
     *
     * @var LocalDate $date
     */
    #[\JMS\Serializer\Annotation\SerializedName('date')]
    public LocalDate $date;

    /**
     * A date-time property.
     *
     * @var \DateTime $dateTime
     */
    #[\JMS\Serializer\Annotation\SerializedName('dateTime')]
    public \DateTime $dateTime;

    /**
     *
     * @var ?float $decimal
     */
    #[\JMS\Serializer\Annotation\SerializedName('decimal')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?float $decimal = null;

    /**
     *
     * @var ?float $decimalNullableOpt
     */
    #[\JMS\Serializer\Annotation\SerializedName('decimalNullableOpt')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?float $decimalNullableOpt = null;

    /**
     *
     * @var ?string $decimalStr
     */
    #[\JMS\Serializer\Annotation\SerializedName('decimalStr')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?string $decimalStr = null;

    /**
     * A string based enum
     *
     * @var Enum $enum
     */
    #[\JMS\Serializer\Annotation\SerializedName('enum')]
    #[\JMS\Serializer\Annotation\Type('\OpenAPI\OpenAPI\Models\Shared\Enum')]
    public Enum $enum;

    /**
     * A float32 property.
     *
     * @var float $float32
     */
    #[\JMS\Serializer\Annotation\SerializedName('float32')]
    public float $float32;

    /**
     * A float64 string
     *
     * @var ?string $float64Str
     */
    #[\JMS\Serializer\Annotation\SerializedName('float64Str')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?string $float64Str = null;

    /**
     * An integer property.
     *
     * @var int $int
     */
    #[\JMS\Serializer\Annotation\SerializedName('int')]
    public int $int;

    /**
     * An int32 property.
     *
     * @var int $int32
     */
    #[\JMS\Serializer\Annotation\SerializedName('int32')]
    public int $int32;

    /**
     * An int32 enum property.
     *
     * @var SimpleObjectWithNonStandardTypeNameInt32Enum $int32Enum
     */
    #[\JMS\Serializer\Annotation\SerializedName('int32Enum')]
    #[\JMS\Serializer\Annotation\Type('\OpenAPI\OpenAPI\Models\Shared\SimpleObjectWithNonStandardTypeNameInt32Enum')]
    public SimpleObjectWithNonStandardTypeNameInt32Enum $int32Enum;

    /**
     * An int64 string
     *
     * @var ?string $int64Str
     */
    #[\JMS\Serializer\Annotation\SerializedName('int64Str')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?string $int64Str = null;

    /**
     * An integer enum property.
     *
     * @var SimpleObjectWithNonStandardTypeNameIntEnum $intEnum
     */
    #[\JMS\Serializer\Annotation\SerializedName('intEnum')]
    #[\JMS\Serializer\Annotation\Type('\OpenAPI\OpenAPI\Models\Shared\SimpleObjectWithNonStandardTypeNameIntEnum')]
    public SimpleObjectWithNonStandardTypeNameIntEnum $intEnum;

    /**
     * An optional integer property will be null for tests.
     *
     * @var ?int $intOptNull
     */
    #[\JMS\Serializer\Annotation\SerializedName('intOptNull')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?int $intOptNull = null;

    /**
     * A number property.
     *
     * @var float $num
     */
    #[\JMS\Serializer\Annotation\SerializedName('num')]
    public float $num;

    /**
     * An optional number property will be null for tests.
     *
     * @var ?float $numOptNull
     */
    #[\JMS\Serializer\Annotation\SerializedName('numOptNull')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?float $numOptNull = null;

    /**
     *
     * @var string $objType
     */
    #[\JMS\Serializer\Annotation\SerializedName('obj.type')]
    public string $objType;

    /**
     * A string property.
     *
     * @var string $str
     */
    #[\JMS\Serializer\Annotation\SerializedName('str')]
    public string $str;

    /**
     * An optional string property.
     *
     * @var ?string $strOpt
     */
    #[\JMS\Serializer\Annotation\SerializedName('strOpt')]
    #[\JMS\Serializer\Annotation\SkipWhenNull]
    public ?string $strOpt = null;

    /**
     * @param  mixed  $any
     * @param  bool  $bool
     * @param  LocalDate  $date
     * @param  \DateTime  $dateTime
     * @param  Enum  $enum
     * @param  float  $float32
     * @param  int  $int
     * @param  int  $int32
     * @param  SimpleObjectWithNonStandardTypeNameInt32Enum  $int32Enum
     * @param  SimpleObjectWithNonStandardTypeNameIntEnum  $intEnum
     * @param  float  $num
     * @param  string  $objType
     * @param  string  $str
     * @param  ?int  $bigint
     * @param  ?string  $bigintStr
     * @param  ?bool  $boolOpt
     * @param  ?float  $decimal
     * @param  ?string  $decimalStr
     * @param  ?string  $float64Str
     * @param  ?string  $int64Str
     * @param  ?int  $intOptNull
     * @param  ?float  $numOptNull
     * @param  ?string  $strOpt
     * @param  ?float  $decimalNullableOpt
     */
    public function __construct(mixed $any, bool $bool, LocalDate $date, \DateTime $dateTime, Enum $enum, float $float32, int $int, int $int32, SimpleObjectWithNonStandardTypeNameInt32Enum $int32Enum, SimpleObjectWithNonStandardTypeNameIntEnum $intEnum, float $num, string $objType, string $str, ?int $bigint = null, ?string $bigintStr = null, ?bool $boolOpt = null, ?float $decimal = null, ?string $decimalStr = null, ?string $float64Str = null, ?string $int64Str = null, ?int $intOptNull = null, ?float $numOptNull = null, ?string $strOpt = null, ?float $decimalNullableOpt = null)
    {
        $this->any = $any;
        $this->bool = $bool;
        $this->date = $date;
        $this->dateTime = $dateTime;
        $this->enum = $enum;
        $this->float32 = $float32;
        $this->int = $int;
        $this->int32 = $int32;
        $this->int32Enum = $int32Enum;
        $this->intEnum = $intEnum;
        $this->num = $num;
        $this->objType = $objType;
        $this->str = $str;
        $this->bigint = $bigint;
        $this->bigintStr = $bigintStr;
        $this->boolOpt = $boolOpt;
        $this->decimal = $decimal;
        $this->decimalStr = $decimalStr;
        $this->float64Str = $float64Str;
        $this->int64Str = $int64Str;
        $this->intOptNull = $intOptNull;
        $this->numOptNull = $numOptNull;
        $this->strOpt = $strOpt;
        $this->decimalNullableOpt = $decimalNullableOpt;
    }
}