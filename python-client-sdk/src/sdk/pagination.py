...
class Pagination:
    ...
    def pagination_limit_offset_deep_outputs_page_body(self, request: shared.LimitOffsetConfig, server_url: Optional[str] = None) -> operations.PaginationLimitOffsetDeepOutputsPageBodyResponse:
        ...
        base_url = utils.template_url(operations.PAGINATION_LIMIT_OFFSET_DEEP_OUTPUTS_PAGE_BODY_SERVERS[0], {
        })
        if server_url is not None:
            base_url = server_url
        ...
        res = operations.PaginationLimitOffsetDeepOutputsPageBodyResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res, next=next_func)
        
        if http_res.status_code == 200:
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[operations.PaginationLimitOffsetDeepOutputsPageBodyRes])
                res.res = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
