...
def stream_events(stream: Iterator[bytes], decoder: Callable[[str], T]) -> Generator[T, None, None]:
    buffer = bytearray()
    position = 0
    for chunk in stream:
        buffer += chunk
...
def _parse_event(raw: bytearray, decoder: Callable[[str], T]):
    ...
    if data:
        ...
        if data.isnumeric() or data == "true" or data == "false" or data == "null" or data.startswith("{") or data.startswith("[") or data.startswith("\""):
            try:
