...
def test_union_map():
    ...
    req = UnionMapRequestBody(input={"str": "test", "bool": True})
    res = s.unions.union_map(request=req)
    assert res is not None
    assert res.http_meta.response.status_code == 200
    assert res.res.json.input["str"] == "test"
    assert res.res.json.input["bool"] == True
