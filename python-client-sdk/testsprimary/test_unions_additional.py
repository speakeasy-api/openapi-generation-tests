...
def test_array_of_discriminated_unions():
    ...
    simpleObject = create_simple_object_with_type()
    deepObject = create_deep_object_with_type()

    res = s.unions.array_of_discriminated_unions(request=[simpleObject, deepObject])
    ...
    compare_simple_object_with_type(res.res.json_[0], simpleObject)
    compare_deep_object_with_type(res.res.json_[1], deepObject)
...
def test_array_of_discriminated_unions_map():
    ...
    req = shared.ArrayOfDiscriminatedUnionsMap(
        array_map={
            "item": [simpleObject, deepObject],
        }
    )

    res = s.unions.array_of_discriminated_unions_map(request=req)
    ...
    assert res.http_meta.response.status_code == 200
    assert res.res is not None
    map = res.res.json_.array_map
    assert isinstance(map, dict) and "item" in map
    item = res.res.json_.array_map.get("item")
    assert len(item) == 2
    compare_simple_object_with_type(item[0], simpleObject)
    compare_deep_object_with_type(item[1], deepObject)
...
def test_nested_array_of_discriminated_unions():
    ...
    req = shared.NestedArrayOfDiscriminatedUnions(
        nested_array=[[simpleObject], [deepObject, simpleObject]]
    )

    res = s.unions.nested_array_of_discriminated_unions(request=req)
    ...
    compare_simple_object_with_type(res.res.json_.nested_array[0][0], simpleObject)
    assert isinstance(res.res.json_.nested_array[1], list)
    assert len(res.res.json_.nested_array[1]) == 2
    compare_deep_object_with_type(res.res.json_.nested_array[1][0], deepObject)
    compare_simple_object_with_type(res.res.json_.nested_array[1][1], simpleObject)
