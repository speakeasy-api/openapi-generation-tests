# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true

require 'faraday'
require 'faraday/multipart'
require 'sorbet-runtime'

module OpenApiSDK
  extend T::Sig
  class Pagination
    extend T::Sig
    
    # PAGINATION_AMBIGUOUS_INPUT_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_AMBIGUOUS_INPUT_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_BODY_FLATTENED_OPTIONAL_SECURITY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_BODY_FLATTENED_OPTIONAL_SECURITY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_BODY_FLATTENED_WITH_SECURITY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_BODY_FLATTENED_WITH_SECURITY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_BODY_WRAPPED_REQUEST_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_BODY_WRAPPED_REQUEST_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_CURSOR_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_CURSOR_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_CURSOR_NON_NUMERIC_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_CURSOR_NON_NUMERIC_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_CURSOR_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_CURSOR_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_ENCAPSULATED_PARAMETER_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_ENCAPSULATED_PARAMETER_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_DEEP_OUTPUTS_PAGE_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_DEEP_OUTPUTS_PAGE_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_OFFSET_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_OFFSET_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_OFFSET_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_OFFSET_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_OPTIONAL_PAGE_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_OPTIONAL_PAGE_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_PAGE_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_PAGE_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_LIMIT_OFFSET_PAGE_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_LIMIT_OFFSET_PAGE_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_URL_PARAMS_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_URL_PARAMS_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_WITH_RETRIES_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_WITH_RETRIES_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    
    # PAGINATION_WRAPPED_OPTIONAL_BODY_SERVERS contains the list of server urls available to the SDK.
    PAGINATION_WRAPPED_OPTIONAL_BODY_SERVERS = [
      'http://localhost:35456', 
    ].freeze
    # Endpoints for testing the pagination extension

    sig { params(sdk_config: SDKConfiguration).void }
    def initialize(sdk_config)
      @sdk_configuration = sdk_config
    end


    sig { params(request_body: ::OpenApiSDK::Operations::PaginationAmbiguousInputRequestBody, cursor: T.nilable(::Integer), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationAmbiguousInputResponse) }
    def pagination_ambiguous_input(request_body, cursor = nil, server_url = nil)
      # pagination_ambiguous_input - This is a paginated operation where there is both a query parameter and
      # request body field called "cursor". This ambiguity is used to test that
      # the generator only updates the appropriate field in the pagination code
      # "next()" function.
      # 
      request = ::OpenApiSDK::Operations::PaginationAmbiguousInputRequest.new(
        
        request_body: request_body,
        cursor: cursor
      )
      base_url = Utils.template_url(PAGINATION_AMBIGUOUS_INPUT_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor#ambiguousInput"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request_body, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationAmbiguousInputRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationAmbiguousInputResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationAmbiguousInputRes)
          res.res = out
        end
      end

      res
    end


    sig { params(security: ::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecuritySecurity, limit: ::Integer, offset: ::Integer, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecurityResponse) }
    def pagination_body_flattened_optional_security(security, limit, offset, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecurityRequest.new(
        
        limit: limit,
        offset: offset
      )
      base_url = Utils.template_url(PAGINATION_BODY_FLATTENED_OPTIONAL_SECURITY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset#paginationBodyFlattenedOptionalSecurity"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecurityRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, security) if !security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecurityResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationBodyFlattenedOptionalSecurityRes)
          res.res = out
        end
      end

      res
    end


    sig { params(security: ::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecuritySecurity, limit: ::Integer, offset: ::Integer, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecurityResponse) }
    def pagination_body_flattened_with_security(security, limit, offset, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecurityRequest.new(
        
        limit: limit,
        offset: offset
      )
      base_url = Utils.template_url(PAGINATION_BODY_FLATTENED_WITH_SECURITY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset#paginationBodyFlattenedWithSecurity"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecurityRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, security) if !security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecurityResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationBodyFlattenedWithSecurityRes)
          res.res = out
        end
      end

      res
    end


    sig { params(limit_offset_config: ::OpenApiSDK::Shared::LimitOffsetConfig, idempotency_key: T.nilable(::String), request_id: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationBodyWrappedRequestResponse) }
    def pagination_body_wrapped_request(limit_offset_config, idempotency_key = nil, request_id = nil, server_url = nil)
      # pagination_body_wrapped_request - This operation has a request wrapper type that encapsulates the
      # parameters and request body. The pagination inputs are meant to go in
      # the request body and we want to test that the generator correctly
      # generates the next() function call preserving everything from the
      # original request and interpolating the next pagination inputs.
      # 
      request = ::OpenApiSDK::Operations::PaginationBodyWrappedRequestRequest.new(
        
        limit_offset_config: limit_offset_config,
        idempotency_key: idempotency_key,
        request_id: request_id
      )
      base_url = Utils.template_url(PAGINATION_BODY_WRAPPED_REQUEST_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/page#paginationBodyWrappedRequest"
      headers = Utils.get_headers(request, @sdk_configuration.globals)
      req_content_type, data, form = Utils.serialize_request_body(request, :limit_offset_config, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationBodyWrappedRequestResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationBodyWrappedRequestRes)
          res.res = out
        end
      end

      res
    end


    sig { params(request: ::OpenApiSDK::Operations::PaginationCursorBodyRequestBody, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationCursorBodyResponse) }
    def pagination_cursor_body(request, server_url = nil)

      base_url = Utils.template_url(PAGINATION_CURSOR_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationCursorBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationCursorBodyRes)
          res.res = out
        end
      end

      res
    end


    sig { params(cursor: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationCursorNonNumericResponse) }
    def pagination_cursor_non_numeric(cursor = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationCursorNonNumericRequest.new(
        
        cursor: cursor
      )
      base_url = Utils.template_url(PAGINATION_CURSOR_NON_NUMERIC_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor_non_numeric"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationCursorNonNumericRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationCursorNonNumericResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationCursorNonNumericRes)
          res.res = out
        end
      end

      res
    end


    sig { params(cursor: ::Integer, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationCursorParamsResponse) }
    def pagination_cursor_params(cursor, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationCursorParamsRequest.new(
        
        cursor: cursor
      )
      base_url = Utils.template_url(PAGINATION_CURSOR_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationCursorParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationCursorParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationCursorParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(cursor: T.nilable(::Integer), header_param: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationEncapsulatedParameterResponse) }
    def pagination_encapsulated_parameter(cursor = nil, header_param = nil, server_url = nil)
      # pagination_encapsulated_parameter - This is a paginated operation where the input is a query parameter
      # but the number of arguments exceeds maxMethodParams so it ends up
      # being encapsulated into a request object.
      # 
      request = ::OpenApiSDK::Operations::PaginationEncapsulatedParameterRequest.new(
        
        cursor: cursor,
        header_param: header_param
      )
      base_url = Utils.template_url(PAGINATION_ENCAPSULATED_PARAMETER_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor#encapsulatedParameter"
      headers = Utils.get_headers(request, @sdk_configuration.globals)
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationEncapsulatedParameterRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationEncapsulatedParameterResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationEncapsulatedParameterRes)
          res.res = out
        end
      end

      res
    end


    sig { params(request: ::OpenApiSDK::Shared::LimitOffsetConfig, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetDeepOutputsPageBodyResponse) }
    def pagination_limit_offset_deep_outputs_page_body(request, server_url = nil)

      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_DEEP_OUTPUTS_PAGE_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/deep_outputs/page"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetDeepOutputsPageBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetDeepOutputsPageBodyRes)
          res.res = out
        end
      end

      res
    end


    sig { params(request: ::OpenApiSDK::Shared::LimitOffsetConfigWithDefaults, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetBodyResponse) }
    def pagination_limit_offset_default_offset_body(request, server_url = nil)

      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset#WithDefaults"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetBodyRes)
          res.res = out
        end
      end

      res
    end


    sig { params(limit: T.nilable(::Integer), offset: T.nilable(::Integer), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetParamsResponse) }
    def pagination_limit_offset_default_offset_params(limit = nil, offset = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetParamsRequest.new(
        
        limit: limit,
        offset: offset
      )
      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_DEFAULT_OFFSET_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset#WithDefaults"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetDefaultOffsetParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(request: ::OpenApiSDK::Shared::LimitOffsetConfig, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetOffsetBodyResponse) }
    def pagination_limit_offset_offset_body(request, server_url = nil)

      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_OFFSET_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetOffsetBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetOffsetBodyRes)
          res.res = out
        end
      end

      res
    end


    sig { params(limit: T.nilable(::Integer), offset: T.nilable(::Integer), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetOffsetParamsResponse) }
    def pagination_limit_offset_offset_params(limit = nil, offset = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationLimitOffsetOffsetParamsRequest.new(
        
        limit: limit,
        offset: offset
      )
      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_OFFSET_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationLimitOffsetOffsetParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetOffsetParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetOffsetParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(page: T.nilable(::Integer), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetOptionalPageParamsResponse) }
    def pagination_limit_offset_optional_page_params(page = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationLimitOffsetOptionalPageParamsRequest.new(
        
        page: page
      )
      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_OPTIONAL_PAGE_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/page#optionalPageParams"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationLimitOffsetOptionalPageParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetOptionalPageParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetOptionalPageParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(request: ::OpenApiSDK::Shared::LimitOffsetConfig, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetPageBodyResponse) }
    def pagination_limit_offset_page_body(request, server_url = nil)

      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_PAGE_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/page"
      headers = {}
      req_content_type, data, form = Utils.serialize_request_body(request, :request, :json)
      headers['content-type'] = req_content_type
      raise StandardError, 'request body is required' if data.nil? && form.nil?
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetPageBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetPageBodyRes)
          res.res = out
        end
      end

      res
    end


    sig { params(page: ::Integer, server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationLimitOffsetPageParamsResponse) }
    def pagination_limit_offset_page_params(page, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationLimitOffsetPageParamsRequest.new(
        
        page: page
      )
      base_url = Utils.template_url(PAGINATION_LIMIT_OFFSET_PAGE_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/page"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationLimitOffsetPageParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationLimitOffsetPageParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationLimitOffsetPageParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(attempts: ::Integer, is_reference_path: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationURLParamsResponse) }
    def pagination_url_params(attempts, is_reference_path = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationURLParamsRequest.new(
        
        attempts: attempts,
        is_reference_path: is_reference_path
      )
      base_url = Utils.template_url(PAGINATION_URL_PARAMS_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/url"
      headers = {}
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationURLParamsRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationURLParamsResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationURLParamsRes)
          res.res = out
        end
      end

      res
    end


    sig { params(cursor: T.nilable(::String), fault_settings: T.nilable(::String), request_id: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationWithRetriesResponse) }
    def pagination_with_retries(cursor = nil, fault_settings = nil, request_id = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationWithRetriesRequest.new(
        
        cursor: cursor,
        fault_settings: fault_settings,
        request_id: request_id
      )
      base_url = Utils.template_url(PAGINATION_WITH_RETRIES_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/cursor_non_numeric#withRetries"
      headers = Utils.get_headers(request, @sdk_configuration.globals)
      query_params = Utils.get_query_params(::OpenApiSDK::Operations::PaginationWithRetriesRequest, request, @sdk_configuration.globals)
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.get(url) do |req|
        req.headers = headers
        req.params = query_params
        Utils.configure_request_security(req, @sdk_configuration.security) if !@sdk_configuration.nil? && !@sdk_configuration.security.nil?
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationWithRetriesResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationWithRetriesRes)
          res.res = out
        end
      end

      res
    end


    sig { params(security: ::OpenApiSDK::Operations::PaginationWrappedOptionalBodySecurity, idempotency_key: T.nilable(::String), limit_offset_config: T.nilable(::OpenApiSDK::Shared::LimitOffsetConfig), request_id: T.nilable(::String), server_url: T.nilable(String)).returns(::OpenApiSDK::Operations::PaginationWrappedOptionalBodyResponse) }
    def pagination_wrapped_optional_body(security, idempotency_key = nil, limit_offset_config = nil, request_id = nil, server_url = nil)

      request = ::OpenApiSDK::Operations::PaginationWrappedOptionalBodyRequest.new(
        
        idempotency_key: idempotency_key,
        limit_offset_config: limit_offset_config,
        request_id: request_id
      )
      base_url = Utils.template_url(PAGINATION_WRAPPED_OPTIONAL_BODY_SERVERS[0], {
      })
      base_url = server_url if !server_url.nil?
      url = "#{base_url}/pagination/limitoffset/offset#paginationWrappedOptionalBody"
      headers = Utils.get_headers(request, @sdk_configuration.globals)
      req_content_type, data, form = Utils.serialize_request_body(request, :limit_offset_config, :json)
      headers['content-type'] = req_content_type
      headers['Accept'] = 'application/json'
      headers['x-speakeasy-user-agent'] = @sdk_configuration.user_agent

      r = @sdk_configuration.client.put(url) do |req|
        req.headers = headers
        Utils.configure_request_security(req, security) if !security.nil?
        if form
          req.body = Utils.encode_form(form)
        elsif Utils.match_content_type(req_content_type, 'application/x-www-form-urlencoded')
          req.body = URI.encode_www_form(data)
        else
          req.body = data
        end
      end

      content_type = r.headers.fetch('Content-Type', 'application/octet-stream')

      res = ::OpenApiSDK::Operations::PaginationWrappedOptionalBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, ::OpenApiSDK::Operations::PaginationWrappedOptionalBodyRes)
          res.res = out
        end
      end

      res
    end
  end
end
