...
module OpenApiSDK
  ...
  class RequestBodies
    ...
    def request_body_post_not_nullable_not_required_string_body(request)
      ...
      res = Operations::RequestBodyPostNotNullableNotRequiredStringBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
    ...
    def request_body_post_nullable_not_required_string_body(request)
      ...
      res = Operations::RequestBodyPostNullableNotRequiredStringBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
    ...
    def request_body_post_nullable_required_string_body(request)
      ...
      res = Operations::RequestBodyPostNullableRequiredStringBodyResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
    ...
    sig { params(request: Operations::RequestBodyPutMultipartDifferentFileNameRequestBody).returns(Utils::FieldAugmented) }
    def request_body_put_multipart_different_file_name(request)
      ...
      res = Operations::RequestBodyPutMultipartDifferentFileNameResponse.new(
        status_code: r.status, content_type: content_type, raw_response: r
      )
      if r.status == 200
        if Utils.match_content_type(content_type, 'application/json')
          out = Utils.unmarshal_complex(r.env.response_body, Operations::RequestBodyPutMultipartDifferentFileNameRes)
          res.res = out
        end
      end
      res
