...
///
/// ``Client`` is the main object that you interact with to communicate with the API, and is responsible for making the underlying HTTP requests for each API operation and handling the resulting responses. It implements the ``OpenAPI`` protocol to support the available API operations.
///
/// ### API calls
///
/// Each API endpoint (or *operation*) is exposed as a method. For instance, the 'ambiguousQueryParam' operation is exposed as:
///
/// ```swift
...
///
/// The ``Response/data`` property defined by each API operation is an enum, and you can `switch` on this value to extract the corresponding value from the response. For example, the 'ambiguousQueryParam' operation returns a ``Operations/AmbiguousQueryParamResponse`` value:
///
/// ```swift
/// let response: Response<Operations.AmbiguousQueryParamResponse> = ... // Response from API call
...
/// ### Server overrides
///
/// By default, API calls are made to `http://localhost:35123`. This can be configured using the ``use(server:)`` method, passing a ``GlobalServers`` value.
///
/// ## Topics
///
/// ### Initializers
...
public final class Client {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            _makeRequest(with: server, configureRequest: configureRequest) { result in
                do {
                    ...
                    switch error {
                    case .internal:
                        continuation.resume(throwing: OpenAPIError.internalError(error: nil))
                    case .missingDefaultServer(let serverType):
                        continuation.resume(throwing: OpenAPIError.failedToConstructRequestURL(.missingDefaultServer(serverType: serverType)))
                    case .invalidServerIndex(let serverType, let index):
                        continuation.resume(throwing: OpenAPIError.failedToConstructRequestURL(.invalidServerIndex(serverType: serverType, index: index)))
                    case .missingServerParameterSubstitutionKey(let key, let serverString):
                        continuation.resume(throwing: OpenAPIError.failedToSerializeRequestParameters(.missingServerParameterSubstitutionKey(key, serverString: serverString)))
                    }
                    ...
                    case .invalidURL(let urlString):
                        continuation.resume(throwing: OpenAPIError.failedToConstructRequestURL(.invalidRequestURL(string: urlString)))
                    case .missingPathParameterSubstitutionKey(let key, let path):
                        continuation.resume(throwing: OpenAPIError.failedToSerializeRequestParameters(.missingPathParameterSubstitutionKey(key, path: path)))
                    }
                } catch let error as SerializationError {
                    switch error {
                    case .failedToSerializeData:
                        continuation.resume(throwing: OpenAPIError.failedToSerializeRequestParameters(.failedToSerializeData))
                    case .missingRequiredRequestBody:
                        continuation.resume(throwing: OpenAPIError.failedToConstructRequest(.missingRequiredRequestBody))
                    case .invalidSerializationParameter(let type, let format):
                        continuation.resume(throwing: OpenAPIError.failedToSerializeRequestParameters(.invalidSerializationParameter(type: type, format: format)))
                    }
                } catch let error as ResponseHandlerError {
                    switch error {
                    case .failedToDecodeResponse:
                        continuation.resume(throwing: OpenAPIError.failedToHandleResponse(.failedToDecodeResponse))
                    case .failedToDecodeJSON(let error):
                        continuation.resume(throwing: OpenAPIError.failedToHandleResponse(.failedToDeserializeJSON(error)))
                    }
                } catch let error as OpenAPIError {
                    continuation.resume(throwing: error)
                } catch {
                    continuation.resume(throwing: OpenAPIError.internalError(error: error))
                }
            }
        }
    }
    ...
    private func baseURL(serverOverride: Server? = nil) throws -> URL {
        ...
        guard let url = URL(string: server.urlString) else {
            throw OpenAPIError.failedToConstructRequestURL(.invalidServerURL(string: server.urlString))
        }
        return url
    }
    ...
    private func makeDataRequest(with urlRequest: URLRequest, completion: @escaping (Result<APIResponse, Swift.Error>) -> Void) {
        let task = _session.dataTask(with: urlRequest) { data, response, error in
            if let error {
                ...
                completion(.failure(OpenAPIError.internalError(error: nil)))
            }
        }
        task.resume()
    }
