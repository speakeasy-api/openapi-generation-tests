...
extension Client: OpenAPI {
    ...
    public func postAdditionalPropertiesWithNullableFields(request: [String: AnyValue]) async throws -> Response<Operations.PostAdditionalPropertiesWithNullableFieldsResponse> {
        return try await makeRequest(
            configureRequest: { configuration in
                try configurePostAdditionalPropertiesWithNullableFieldsRequest(with: configuration, request: request)
            },
            handleResponse: handlePostAdditionalPropertiesWithNullableFieldsResponse
        )
    }
    ...
    public var health: HealthAPI {
        return _HealthAPI(client: self)
    }
    ...
    public var statusCode: StatusCodeAPI {
        return _StatusCodeAPI(client: self)
    }
}
...
private func configurePostAdditionalPropertiesWithNullableFieldsRequest(with configuration: URLRequestConfiguration, request: [String: AnyValue]) throws {
    configuration.path = "/anything/additionalPropertiesWithNullableFields"
    configuration.method = .post
    configuration.contentType = "application/json"
    configuration.body = try jsonEncoder().encode(request)
    if configuration.body == nil {
...
private func handlePostAdditionalPropertiesWithNullableFieldsResponse(response: Client.APIResponse) throws -> Operations.PostAdditionalPropertiesWithNullableFieldsResponse {
    let httpResponse = response.httpResponse
    
    if httpResponse.statusCode == 200 { 
        if httpResponse.contentType.matchContentType(pattern: "application/json"), let data = response.data {
            do {
                return .object(try JSONDecoder().decode(Operations.PostAdditionalPropertiesWithNullableFieldsResponseBody.self, from: data))
            } catch {
                throw ResponseHandlerError.failedToDecodeJSON(error)
            }
        }
