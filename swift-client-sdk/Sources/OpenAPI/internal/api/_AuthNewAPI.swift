...
class _AuthNewAPI: AuthNewAPI {
    ...
    public func customSchemeAppId(security: Operations.CustomSchemeAppIdSecurity, server: AuthNewServers.CustomSchemeAppId?) async throws -> Response<Operations.CustomSchemeAppIdResponse> {
        return try await client.makeRequest(
            with: try server?.server() ?? AuthNewServers.CustomSchemeAppId.default(),
            configureRequest: { configuration in
                try configureCustomSchemeAppIdRequest(with: configuration, security: security)
            },
            handleResponse: handleCustomSchemeAppIdResponse
        )
    }
...
private func configureCustomSchemeAppIdRequest(with configuration: URLRequestConfiguration, security: Operations.CustomSchemeAppIdSecurity) throws {
    configuration.path = "/auth/customsecurity/customSchemeAppId"
...
private func handleCustomSchemeAppIdResponse(response: Client.APIResponse) throws -> Operations.CustomSchemeAppIdResponse {
    let httpResponse = response.httpResponse
