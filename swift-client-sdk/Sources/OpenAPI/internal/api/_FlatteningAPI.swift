...
class _FlatteningAPI: FlatteningAPI {
    ...
    public func requiredBodyAllOptional(request: Shared.ObjWithOptionalProperties) async throws -> Response<Operations.RequiredBodyAllOptionalResponse> {
        return try await client.makeRequest(
            configureRequest: { configuration in
                try configureRequiredBodyAllOptionalRequest(with: configuration, request: request)
            },
            handleResponse: handleRequiredBodyAllOptionalResponse
        )
    }
...
private func configureRequiredBodyAllOptionalRequest(with configuration: URLRequestConfiguration, request: Shared.ObjWithOptionalProperties) throws {
    configuration.path = "/anything/flattening/requiredBodyAllOptional"
    configuration.method = .post
    configuration.contentType = "application/json"
...
private func handleRequiredBodyAllOptionalResponse(response: Client.APIResponse) throws -> Operations.RequiredBodyAllOptionalResponse {
    let httpResponse = response.httpResponse
    
    if httpResponse.statusCode == 200 { 
        if httpResponse.contentType.matchContentType(pattern: "application/json"), let data = response.data {
            do {
                return .res(try JSONDecoder().decode(Operations.RequiredBodyAllOptionalRes.self, from: data))
            } catch {
                throw ResponseHandlerError.failedToDecodeJSON(error)
            }
        }
