...
class _HooksAPI: HooksAPI {
    ...
    public func testHooksBeforeCreateRequestPaths(request: Operations.TestHooksBeforeCreateRequestPathsRequest) async throws -> Response<Operations.TestHooksBeforeCreateRequestPathsResponse> {
        return try await client.makeRequest(
            configureRequest: { configuration in
                try configureTestHooksBeforeCreateRequestPathsRequest(with: configuration, request: request)
            },
            handleResponse: handleTestHooksBeforeCreateRequestPathsResponse
        )
    }
...
private func configureTestHooksBeforeCreateRequestPathsRequest(with configuration: URLRequestConfiguration, request: Operations.TestHooksBeforeCreateRequestPathsRequest) throws {
    configuration.path = "/anything/hooks/beforeCreateRequestPaths/{namespace}"
    configuration.method = .get
...
private func handleTestHooksBeforeCreateRequestPathsResponse(response: Client.APIResponse) throws -> Operations.TestHooksBeforeCreateRequestPathsResponse {
    let httpResponse = response.httpResponse
    
    if httpResponse.statusCode == 200 { 
        if httpResponse.contentType.matchContentType(pattern: "application/json"), let data = response.data {
            do {
                return .res(try JSONDecoder().decode(Operations.TestHooksBeforeCreateRequestPathsRes.self, from: data))
            } catch {
                throw ResponseHandlerError.failedToDecodeJSON(error)
            }
        }
