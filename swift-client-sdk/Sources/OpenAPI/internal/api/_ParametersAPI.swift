...
class _ParametersAPI: ParametersAPI {
    ...
    public func headerParamsNil(request: Operations.HeaderParamsNilRequest) async throws -> Response<Operations.HeaderParamsNilResponse> {
        return try await client.makeRequest(
            configureRequest: { configuration in
                try configureHeaderParamsNilRequest(with: configuration, request: request)
            },
            handleResponse: handleHeaderParamsNilResponse
        )
    }
...
private func configureHeaderParamsNilRequest(with configuration: URLRequestConfiguration, request: Operations.HeaderParamsNilRequest) throws {
    configuration.path = "/anything/headers/nil"
    configuration.method = .get
    configuration.headerParameterSerializable = request
    configuration.telemetryHeader = .speakeasyUserAgent
}
...
private func handleHeaderParamsNilResponse(response: Client.APIResponse) throws -> Operations.HeaderParamsNilResponse {
    let httpResponse = response.httpResponse
    
    if httpResponse.statusCode == 200 { 
        if httpResponse.contentType.matchContentType(pattern: "application/json"), let data = response.data {
            do {
                return .res(try JSONDecoder().decode(Operations.HeaderParamsNilRes.self, from: data))
            } catch {
                throw ResponseHandlerError.failedToDecodeJSON(error)
            }
        }
