...
class _ParametersAPI: ParametersAPI {
    ...
    public func pathEncoding(request: Operations.PathEncodingRequest) async throws -> Response<Operations.PathEncodingResponse> {
        return try await client.makeRequest(
            configureRequest: { configuration in
                try configurePathEncodingRequest(with: configuration, request: request)
            },
            handleResponse: handlePathEncodingResponse
        )
    }
...
private func configurePathEncodingRequest(with configuration: URLRequestConfiguration, request: Operations.PathEncodingRequest) throws {
    configuration.path = "/pathencoding/{param1}/{param2}"
    configuration.method = .get
    configuration.pathParameterSerializable = request
    configuration.telemetryHeader = .speakeasyUserAgent
}
...
private func handlePathEncodingResponse(response: Client.APIResponse) throws -> Operations.PathEncodingResponse {
    let httpResponse = response.httpResponse
