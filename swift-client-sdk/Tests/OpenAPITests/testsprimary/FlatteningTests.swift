...
class FlatteningTests: XCTestCase {
    ...
    func testFlatteningRequiredBodyAllOptional() async throws {
        try recordTest(named: "flattening-required-body-all-optional")

        let response = try await client.flattening.requiredBodyAllOptional(
          request: .init(
              optInt: 1,
              optStr: "body test"
            )
        ...
        XCTAssertEqual(response.statusCode, 200, "Request should succeed")
        XCTAssertEqual(try response.data.res().json.optStr, "body test")
        XCTAssertEqual(try response.data.res().json.optInt, 1)

        // `request` is non-optional and passing `request: nil` would throw a compile-time error:
        // 'nil' is not compatible with expected argument type 'Shared.ObjWithOptionalProperties'
    }
}
