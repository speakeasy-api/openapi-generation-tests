...
| Error Object      | Status Code       | Content Type      |
| ----------------- | ----------------- | ----------------- |
| errors.ErrorType1 | 500               | application/json  |
| errors.ErrorType2 | 500               | application/json  |
| errors.SDKError   | 4xx-5xx           | */*               |
...
async function run() {
  const res = await errorsStatusGetError(sdk, 458364);
...
async function run() {
  const res = await errorsStatusGetXSpeakeasyErrors(sdk, 385913);
