...
async function run() {
  const res = await requestBodiesRequestBodyPutBytes(sdk, new TextEncoder().encode("0x9D7A157f89"));
...
async function run() {
  const res = await requestBodiesRequestBodyPutBytesWithParams(sdk, new TextEncoder().encode("0x4Ba445cd92"), "<value>");
