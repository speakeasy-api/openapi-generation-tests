...
async function run() {
  const result = await sdk.requestBodies.nullableRequiredPropertyPost({
    ...
    nullableRequiredBigIntStr: BigInt("235517"),
    nullableRequiredDateTime: new Date("2024-01-29T03:32:22.127Z"),
    nullableRequiredDecimalStr: new Decimal("502.66"),
    nullableRequiredEnum: "first",
    nullableRequiredInt: 843377,
  });
...
async function run() {
  const result = await sdk.requestBodies.requestBodyPutBytesWithParams(await openAsBlob("example.file"), "<value>");
...
async function run() {
  const res = await requestBodiesRequestBodyPutBytesWithParams(sdk, await openAsBlob("example.file"), "<value>");
