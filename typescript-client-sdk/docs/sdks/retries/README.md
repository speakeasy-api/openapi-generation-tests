...
async function run() {
  ...
  const numRetries = 282943;
  const retryAfterVal = 861810;
  
  const result = await sdk.retries.retriesAfter(requestId, numRetries, retryAfterVal);
...
async function run() {
  ...
  const numRetries = 138258;
  
  const result = await sdk.retries.retriesPost(requestId, requestBody, numRetries);
