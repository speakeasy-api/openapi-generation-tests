...
import { SDK } from "..";
import { Security } from "../sdk/models/shared";
...
test("function callbacks for oAuth support are invoked, with param overrides", async () => {
  ...
  expect(res.httpMeta.response.status).toBe(200); // asserts local is used
  expect(res.token?.token).toBe("overrideHeaders");

  res = await sdk.auth.oauth2Override(
    { oauth2: "Bearer overrideHeaders" },
    { fetchOptions: { headers: { Authorization: "Bearer overrideFetch" } } },
  );

  expect(res.httpMeta.response.status).toBe(200); // asserts local is used
  expect(res.token?.token).toBe("overrideFetch");
});
