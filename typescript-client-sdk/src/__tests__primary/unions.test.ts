...
test("typed object one of post null", async () => {
  ...
  try {
    ...
    if (!(err instanceof ZodError)) {
      expect.unreachable("SDK call should have failed with a ZodError");
    }

    expect(err.issues).toHaveLength(1);
    expect(err.issues[0]?.code).toEqual("invalid_union");
  }
});
...
test("union bigint decimal", async () => {
  recordTest("unions-bigint-decimal");

  const sdk = new SDK();

  const dec = new Decimal("3.141592653589793");
  let result = await sdk.unions.unionBigIntDecimal(dec);
  ...
  result = await sdk.unions.unionBigIntDecimal(big);
