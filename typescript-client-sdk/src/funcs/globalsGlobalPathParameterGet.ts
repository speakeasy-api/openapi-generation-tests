...
export async function globalsGlobalPathParameterGet(
  client: SDKCore,
...
> {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GlobalPathParameterGetRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    globalPathParam: encodeSimple(
      "globalPathParam",
      payload.globalPathParam ?? client._options.globalPathParam,
      { explode: false, charEncoding: "percent" },
    ),
  };

  const path = pathToFunc("/anything/globals/pathParameter/{globalPathParam}")(
    pathParams,
  );
