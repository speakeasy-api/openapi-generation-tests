...
export async function requestBodiesNullableOptionalFieldsPost(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.NullableOptionalFieldsPostRequestBody$outboundSchema.parse(
        value,
      ),
  ...
  >(
    M.json(
      200,
      operations.NullableOptionalFieldsPostResponseBody$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
