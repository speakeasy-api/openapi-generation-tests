...
export async function requestBodiesNullableRequiredPropertyPost(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.NullableRequiredPropertyPostRequestBody$outboundSchema.parse(
        value,
      ),
  ...
  >(
    M.json(
      200,
      operations.NullableRequiredPropertyPostResponseBody$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
