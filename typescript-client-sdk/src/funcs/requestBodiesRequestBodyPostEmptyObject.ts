...
export async function requestBodiesRequestBodyPostEmptyObject(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.RequestBodyPostEmptyObjectRequestBody$outboundSchema.parse(
        value,
      ),
  ...
  >(
    M.json(
      200,
      operations.RequestBodyPostEmptyObjectResponseBody$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
