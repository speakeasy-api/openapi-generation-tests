...
export async function requestBodiesRequestBodyPostJsonDataTypesDate(
  client: SDKCore,
  request: RFCDate,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.instanceof(RFCDate).transform(v => v.toString()).parse(value),
  ...
  >(
    M.json(
      200,
      operations.RequestBodyPostJsonDataTypesDateResponseBody$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
