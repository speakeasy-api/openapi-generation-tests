...
export async function requestBodiesRequestBodyPostNullDictionary(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) => z.nullable(z.record(z.string())).parse(value),
  ...
  >(
    M.json(
      200,
      operations.RequestBodyPostNullDictionaryResponseBody$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
