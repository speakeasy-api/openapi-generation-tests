...
> {
  ...
  if (payload.differentFileName !== undefined) {
    if (isBlobLike(payload.differentFileName)) {
      appendForm(body, "differentFileName", payload.differentFileName);
    } else if (isReadableStream(payload.differentFileName.content)) {
      const buffer = await readableStreamToArrayBuffer(
        payload.differentFileName.content,
      );
      const blob = new Blob([buffer], { type: "application/octet-stream" });
      appendForm(body, "differentFileName", blob);
    } else {
      appendForm(
        body,
        "differentFileName",
  ...
  >(
    M.json(
      200,
      operations.RequestBodyPutMultipartDifferentFileNameRes$inboundSchema,
    ),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
