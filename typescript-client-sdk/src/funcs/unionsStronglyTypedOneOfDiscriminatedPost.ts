...
export async function unionsStronglyTypedOneOfDiscriminatedPost(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      shared.StronglyTypedOneOfDiscriminatedObject$outboundSchema.parse(value),
  ...
  >(
    M.json(
      200,
      operations.StronglyTypedOneOfDiscriminatedPostRes$inboundSchema,
    ),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
