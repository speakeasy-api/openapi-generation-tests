/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { SDKHooks } from "../hooks/hooks.js";
import { SDKOptions, serverURLFromOptions } from "../lib/config.js";
import {
    encodeFormQuery as encodeFormQuery$,
    encodeSimple as encodeSimple$,
} from "../lib/encodings.js";
import { HTTPClient } from "../lib/http.js";
import * as schemas$ from "../lib/schemas.js";
import { ClientSDK, RequestOptions } from "../lib/sdks.js";
import * as errors from "./models/errors/index.js";
import { ConnectionErrorGetServerList } from "./models/operations/connectionerrorget.js";
import * as operations from "./models/operations/index.js";
import { StatusGetXSpeakeasyErrorsServerList } from "./models/operations/statusgetxspeakeasyerrors.js";
import { UnionErrorsDiscriminatedGetServerList } from "./models/operations/unionerrorsdiscriminatedget.js";
import { UnionErrorsGetServerList } from "./models/operations/unionerrorsget.js";
import * as z from "zod";

export class Errors extends ClientSDK {
    private readonly options$: SDKOptions & { hooks?: SDKHooks };

    constructor(options: SDKOptions = {}) {
        const opt = options as unknown;
        let hooks: SDKHooks;
        if (
            typeof opt === "object" &&
            opt != null &&
            "hooks" in opt &&
            opt.hooks instanceof SDKHooks
        ) {
            hooks = opt.hooks;
        } else {
            hooks = new SDKHooks();
        }

        super({
            client: options.httpClient || new HTTPClient(),
            baseURL: serverURLFromOptions(options),
            hooks,
        });

        this.options$ = { ...options, hooks };
        void this.options$;
    }

    async connectionErrorGet(options?: RequestOptions & { serverURL?: string }): Promise<void> {
        const baseURL$ =
            options?.serverURL ||
            this.templateURLComponent(ConnectionErrorGetServerList[0], {
                charEncoding: "percent",
            })();

        const path$ = this.templateURLComponent("/anything/connectionError")();

        const query$ = "";

        const headers$ = new Headers({
            Accept: "*/*",
        });

        const security$ =
            typeof this.options$.security === "function"
                ? await this.options$.security()
                : this.options$.security;

        const context = {
            operationID: "connectionErrorGet",
            oAuth2Scopes: [],
            securitySource: this.options$.security,
        };
        const securitySettings$ = this.resolveGlobalSecurity(security$);

        const request$ = this.createRequest$(
            context,
            {
                security: securitySettings$,
                method: "GET",
                baseURL: baseURL$,
                path: path$,
                headers: headers$,
                query: query$,
                uaHeader: "x-speakeasy-user-agent",
                timeoutMs: options?.timeoutMs || this.options$.timeoutMs || -1,
            },
            options
        );

        const response = await this.do$(request$, {
            context,
            errorCodes: ["4XX", "5XX"],
            retryConfig: options?.retries || this.options$.retryConfig,
            retryCodes: options?.retryCodes || ["429", "500", "502", "503", "504"],
        });

        const [result$] = await this.matcher<void>()
            .void(200, z.void())
            .fail(["4XX", "5XX"])
            .match(response);

        return result$;
    }

    async statusGetError(statusCode: number, options?: RequestOptions): Promise<void> {
        const input$: operations.StatusGetErrorRequest = {
            statusCode: statusCode,
        };

        const payload$ = schemas$.parse(
            input$,
            (value$) => operations.StatusGetErrorRequest$outboundSchema.parse(value$),
            "Input validation failed"
        );
        const body$ = null;

        const pathParams$ = {
            statusCode: encodeSimple$("statusCode", payload$.statusCode, {
                explode: false,
                charEncoding: "percent",
            }),
        };
        const path$ = this.templateURLComponent("/status/{statusCode}")(pathParams$);

        const query$ = "";

        const headers$ = new Headers({
            Accept: "*/*",
        });

        const security$ =
            typeof this.options$.security === "function"
                ? await this.options$.security()
                : this.options$.security;

        const context = {
            operationID: "statusGetError",
            oAuth2Scopes: [],
            securitySource: this.options$.security,
        };
        const securitySettings$ = this.resolveGlobalSecurity(security$);

        const request$ = this.createRequest$(
            context,
            {
                security: securitySettings$,
                method: "GET",
                path: path$,
                headers: headers$,
                query: query$,
                body: body$,
                uaHeader: "x-speakeasy-user-agent",
                timeoutMs: options?.timeoutMs || this.options$.timeoutMs || -1,
            },
            options
        );

        const response = await this.do$(request$, {
            context,
            errorCodes: ["400", "4XX", "500", "5XX"],
            retryConfig: options?.retries || this.options$.retryConfig,
            retryCodes: options?.retryCodes || ["429", "500", "502", "503", "504"],
        });

        const [result$] = await this.matcher<void>()
            .void([200, 300], z.void())
            .fail([400, "4XX", 500, "5XX"])
            .match(response);

        return result$;
    }

    async statusGetXSpeakeasyErrors(
        statusCode: number,
        options?: RequestOptions & { serverURL?: string }
    ): Promise<void> {
        const input$: operations.StatusGetXSpeakeasyErrorsRequest = {
            statusCode: statusCode,
        };

        const payload$ = schemas$.parse(
            input$,
            (value$) => operations.StatusGetXSpeakeasyErrorsRequest$outboundSchema.parse(value$),
            "Input validation failed"
        );
        const body$ = null;
        const baseURL$ =
            options?.serverURL ||
            this.templateURLComponent(StatusGetXSpeakeasyErrorsServerList[0], {
                charEncoding: "percent",
            })();

        const pathParams$ = {
            statusCode: encodeSimple$("statusCode", payload$.statusCode, {
                explode: false,
                charEncoding: "percent",
            }),
        };
        const path$ = this.templateURLComponent("/errors/{statusCode}")(pathParams$);

        const query$ = "";

        const headers$ = new Headers({
            Accept: "application/json",
        });

        const security$ =
            typeof this.options$.security === "function"
                ? await this.options$.security()
                : this.options$.security;

        const context = {
            operationID: "statusGetXSpeakeasyErrors",
            oAuth2Scopes: [],
            securitySource: this.options$.security,
        };
        const securitySettings$ = this.resolveGlobalSecurity(security$);

        const request$ = this.createRequest$(
            context,
            {
                security: securitySettings$,
                method: "GET",
                baseURL: baseURL$,
                path: path$,
                headers: headers$,
                query: query$,
                body: body$,
                uaHeader: "x-speakeasy-user-agent",
                timeoutMs: options?.timeoutMs || this.options$.timeoutMs || -1,
            },
            options
        );

        const response = await this.do$(request$, {
            context,
            errorCodes: ["400", "401", "4XX", "500", "501"],
            retryConfig: options?.retries || this.options$.retryConfig,
            retryCodes: options?.retryCodes || ["429", "500", "502", "503", "504"],
        });

        const responseFields$ = {
            HttpMeta: { Response: response, Request: request$ },
        };

        const [result$] = await this.matcher<void>()
            .void([200, 300], z.void())
            .fail([400, 401, "4XX"])
            .json(500, errors.ErrorT$inboundSchema, { err: true })
            .json(501, errors.StatusGetXSpeakeasyErrorsResponseBody$inboundSchema, { err: true })
            .match(response, { extraFields: responseFields$ });

        return result$;
    }

    async unionErrorsDiscriminatedGet(
        errorTag: operations.ErrorTag,
        options?: RequestOptions & { serverURL?: string }
    ): Promise<void> {
        const input$: operations.UnionErrorsDiscriminatedGetRequest = {
            errorTag: errorTag,
        };

        const payload$ = schemas$.parse(
            input$,
            (value$) => operations.UnionErrorsDiscriminatedGetRequest$outboundSchema.parse(value$),
            "Input validation failed"
        );
        const body$ = null;
        const baseURL$ =
            options?.serverURL ||
            this.templateURLComponent(UnionErrorsDiscriminatedGetServerList[0], {
                charEncoding: "percent",
            })();

        const path$ = this.templateURLComponent("/errors/unionErrorsDiscriminated")();

        const query$ = encodeFormQuery$({
            errorTag: payload$.errorTag,
        });

        const headers$ = new Headers({
            Accept: "application/json",
        });

        const security$ =
            typeof this.options$.security === "function"
                ? await this.options$.security()
                : this.options$.security;

        const context = {
            operationID: "unionErrorsDiscriminatedGet",
            oAuth2Scopes: [],
            securitySource: this.options$.security,
        };
        const securitySettings$ = this.resolveGlobalSecurity(security$);

        const request$ = this.createRequest$(
            context,
            {
                security: securitySettings$,
                method: "GET",
                baseURL: baseURL$,
                path: path$,
                headers: headers$,
                query: query$,
                body: body$,
                uaHeader: "x-speakeasy-user-agent",
                timeoutMs: options?.timeoutMs || this.options$.timeoutMs || -1,
            },
            options
        );

        const response = await this.do$(request$, {
            context,
            errorCodes: ["4XX", "5XX"],
            retryConfig: options?.retries || this.options$.retryConfig,
            retryCodes: options?.retryCodes || ["429", "500", "502", "503", "504"],
        });

        const responseFields$ = {
            HttpMeta: { Response: response, Request: request$ },
        };

        const [result$] = await this.matcher<void>()
            .void(200, z.void())
            .fail("4XX")
            .json("5XX", errors.UnionErrorsDiscriminatedGetResponseBody$inboundSchema, {
                err: true,
            })
            .match(response, { extraFields: responseFields$ });

        return result$;
    }

    async unionErrorsGet(
        errorType: operations.ErrorType,
        options?: RequestOptions & { serverURL?: string }
    ): Promise<void> {
        const input$: operations.UnionErrorsGetRequest = {
            errorType: errorType,
        };

        const payload$ = schemas$.parse(
            input$,
            (value$) => operations.UnionErrorsGetRequest$outboundSchema.parse(value$),
            "Input validation failed"
        );
        const body$ = null;
        const baseURL$ =
            options?.serverURL ||
            this.templateURLComponent(UnionErrorsGetServerList[0], { charEncoding: "percent" })();

        const path$ = this.templateURLComponent("/errors/unionErrors")();

        const query$ = encodeFormQuery$({
            errorType: payload$.errorType,
        });

        const headers$ = new Headers({
            Accept: "application/json",
        });

        const security$ =
            typeof this.options$.security === "function"
                ? await this.options$.security()
                : this.options$.security;

        const context = {
            operationID: "unionErrorsGet",
            oAuth2Scopes: [],
            securitySource: this.options$.security,
        };
        const securitySettings$ = this.resolveGlobalSecurity(security$);

        const request$ = this.createRequest$(
            context,
            {
                security: securitySettings$,
                method: "GET",
                baseURL: baseURL$,
                path: path$,
                headers: headers$,
                query: query$,
                body: body$,
                uaHeader: "x-speakeasy-user-agent",
                timeoutMs: options?.timeoutMs || this.options$.timeoutMs || -1,
            },
            options
        );

        const response = await this.do$(request$, {
            context,
            errorCodes: ["4XX", "500", "5XX"],
            retryConfig: options?.retries || this.options$.retryConfig,
            retryCodes: options?.retryCodes || ["429", "500", "502", "503", "504"],
        });

        const responseFields$ = {
            HttpMeta: { Response: response, Request: request$ },
        };

        const [result$] = await this.matcher<void>()
            .void(200, z.void())
            .json(500, errors.UnionErrorsGetResponseBody$inboundSchema, { err: true })
            .fail(["4XX", "5XX"])
            .match(response, { extraFields: responseFields$ });

        return result$;
    }
}
