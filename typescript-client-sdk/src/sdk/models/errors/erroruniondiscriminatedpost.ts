...
export function errorUnionDiscriminatedPostResponseBodyToJSON(
  errorUnionDiscriminatedPostResponseBody:
    ErrorUnionDiscriminatedPostResponseBody,
): string {
  return JSON.stringify(
    ErrorUnionDiscriminatedPostResponseBody$outboundSchema.parse(
      errorUnionDiscriminatedPostResponseBody,
    ),
  );
}

export function errorUnionDiscriminatedPostResponseBodyFromJSON(
  jsonString: string,
): SafeParseResult<
  ErrorUnionDiscriminatedPostResponseBody,
  SDKValidationError
> {
  return safeParse(
    jsonString,
    (x) =>
      ErrorUnionDiscriminatedPostResponseBody$inboundSchema.parse(
        JSON.parse(x),
      ),
    `Failed to parse 'ErrorUnionDiscriminatedPostResponseBody' from JSON`,
  );
}
