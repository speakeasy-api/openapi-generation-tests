...
/** @internal */
export const ChatRequestBody$inboundSchema: z.ZodType<
  ChatRequestBody,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function chatRequestBodyToJSON(
  chatRequestBody: ChatRequestBody,
): string {
  return JSON.stringify(ChatRequestBody$outboundSchema.parse(chatRequestBody));
}

export function chatRequestBodyFromJSON(
  jsonString: string,
): SafeParseResult<ChatRequestBody, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ChatRequestBody$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ChatRequestBody' from JSON`,
  );
}
...
export type ChatResponse$Outbound =
  | Array<shared.ChatCompletionResult$Outbound>
  | never;
