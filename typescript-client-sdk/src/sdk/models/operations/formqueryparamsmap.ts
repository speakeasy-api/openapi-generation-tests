/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import { z } from "zod";

export type FormQueryParamsMapRequest = {
    mapParam?: Record<string, string> | undefined;
    mapParamExploded?: Record<string, number> | undefined;
};

/**
 * OK
 */
export type FormQueryParamsMapRes = {
    args: Record<string, string>;
    url: string;
};

export type FormQueryParamsMapResponse = {
    /**
     * HTTP response content type for this operation
     */
    contentType: string;
    /**
     * HTTP response status code for this operation
     */
    statusCode: number;
    /**
     * Raw HTTP response; suitable for custom response parsing
     */
    rawResponse: Response;
    /**
     * OK
     */
    res?: FormQueryParamsMapRes | undefined;
};

/** @internal */
export namespace FormQueryParamsMapRequest$ {
    export type Inbound = {
        mapParam?: Record<string, string> | undefined;
        mapParamExploded?: Record<string, number> | undefined;
    };

    export const inboundSchema: z.ZodType<FormQueryParamsMapRequest, z.ZodTypeDef, Inbound> = z
        .object({
            mapParam: z.record(z.string()).optional(),
            mapParamExploded: z.record(z.number().int()).optional(),
        })
        .transform((v) => {
            return {
                ...(v.mapParam === undefined ? null : { mapParam: v.mapParam }),
                ...(v.mapParamExploded === undefined
                    ? null
                    : { mapParamExploded: v.mapParamExploded }),
            };
        });

    export type Outbound = {
        mapParam?: Record<string, string> | undefined;
        mapParamExploded?: Record<string, number> | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, FormQueryParamsMapRequest> = z
        .object({
            mapParam: z.record(z.string()).optional(),
            mapParamExploded: z.record(z.number().int()).optional(),
        })
        .transform((v) => {
            return {
                ...(v.mapParam === undefined ? null : { mapParam: v.mapParam }),
                ...(v.mapParamExploded === undefined
                    ? null
                    : { mapParamExploded: v.mapParamExploded }),
            };
        });
}

/** @internal */
export namespace FormQueryParamsMapRes$ {
    export type Inbound = {
        args: Record<string, string>;
        url: string;
    };

    export const inboundSchema: z.ZodType<FormQueryParamsMapRes, z.ZodTypeDef, Inbound> = z
        .object({
            args: z.record(z.string()),
            url: z.string(),
        })
        .transform((v) => {
            return {
                args: v.args,
                url: v.url,
            };
        });

    export type Outbound = {
        args: Record<string, string>;
        url: string;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, FormQueryParamsMapRes> = z
        .object({
            args: z.record(z.string()),
            url: z.string(),
        })
        .transform((v) => {
            return {
                args: v.args,
                url: v.url,
            };
        });
}

/** @internal */
export namespace FormQueryParamsMapResponse$ {
    export type Inbound = {
        ContentType: string;
        StatusCode: number;
        RawResponse: Response;
        res?: FormQueryParamsMapRes$.Inbound | undefined;
    };

    export const inboundSchema: z.ZodType<FormQueryParamsMapResponse, z.ZodTypeDef, Inbound> = z
        .object({
            ContentType: z.string(),
            StatusCode: z.number().int(),
            RawResponse: z.instanceof(Response),
            res: z.lazy(() => FormQueryParamsMapRes$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                contentType: v.ContentType,
                statusCode: v.StatusCode,
                rawResponse: v.RawResponse,
                ...(v.res === undefined ? null : { res: v.res }),
            };
        });

    export type Outbound = {
        ContentType: string;
        StatusCode: number;
        RawResponse: never;
        res?: FormQueryParamsMapRes$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, FormQueryParamsMapResponse> = z
        .object({
            contentType: z.string(),
            statusCode: z.number().int(),
            rawResponse: z.instanceof(Response).transform(() => {
                throw new Error("Response cannot be serialized");
            }),
            res: z.lazy(() => FormQueryParamsMapRes$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                ContentType: v.contentType,
                StatusCode: v.statusCode,
                RawResponse: v.rawResponse,
                ...(v.res === undefined ? null : { res: v.res }),
            };
        });
}
