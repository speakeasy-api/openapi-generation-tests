...
export function globalBearerAuthTokenToJSON(
  globalBearerAuthToken: GlobalBearerAuthToken,
): string {
  return JSON.stringify(
    GlobalBearerAuthToken$outboundSchema.parse(globalBearerAuthToken),
  );
}

export function globalBearerAuthTokenFromJSON(
  jsonString: string,
): SafeParseResult<GlobalBearerAuthToken, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => GlobalBearerAuthToken$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'GlobalBearerAuthToken' from JSON`,
  );
}
