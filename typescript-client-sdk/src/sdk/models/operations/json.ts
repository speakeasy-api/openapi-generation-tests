...
export function jsonResponseToJSON(jsonResponse: JsonResponse): string {
  return JSON.stringify(JsonResponse$outboundSchema.parse(jsonResponse));
}

export function jsonResponseFromJSON(
  jsonString: string,
): SafeParseResult<JsonResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => JsonResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'JsonResponse' from JSON`,
  );
}
