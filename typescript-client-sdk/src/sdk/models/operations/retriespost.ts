/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as z from "zod";

export const RetriesPostServerList = ["http://localhost:35456"] as const;

export type RetriesPostRequestBody = {
    fieldOne: string;
};

export type RetriesPostRequest = {
    requestId: string;
    requestBody?: RetriesPostRequestBody | undefined;
    numRetries?: number | undefined;
};

/**
 * OK
 */
export type RetriesPostRetries = {
    retries: number;
};

export type RetriesPostResponse = {
    /**
     * HTTP response content type for this operation
     */
    contentType: string;
    /**
     * HTTP response status code for this operation
     */
    statusCode: number;
    /**
     * Raw HTTP response; suitable for custom response parsing
     */
    rawResponse: Response;
    /**
     * OK
     */
    retries?: RetriesPostRetries | undefined;
};

/** @internal */
export namespace RetriesPostRequestBody$ {
    export type Inbound = {
        field_one: string;
    };

    export const inboundSchema: z.ZodType<RetriesPostRequestBody, z.ZodTypeDef, Inbound> = z
        .object({
            field_one: z.string(),
        })
        .transform((v) => {
            return {
                fieldOne: v.field_one,
            };
        });

    export type Outbound = {
        field_one: string;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, RetriesPostRequestBody> = z
        .object({
            fieldOne: z.string(),
        })
        .transform((v) => {
            return {
                field_one: v.fieldOne,
            };
        });
}

/** @internal */
export namespace RetriesPostRequest$ {
    export type Inbound = {
        "request-id": string;
        RequestBody?: RetriesPostRequestBody$.Inbound | undefined;
        "num-retries"?: number | undefined;
    };

    export const inboundSchema: z.ZodType<RetriesPostRequest, z.ZodTypeDef, Inbound> = z
        .object({
            "request-id": z.string(),
            RequestBody: z.lazy(() => RetriesPostRequestBody$.inboundSchema).optional(),
            "num-retries": z.number().int().optional(),
        })
        .transform((v) => {
            return {
                requestId: v["request-id"],
                ...(v.RequestBody === undefined ? null : { requestBody: v.RequestBody }),
                ...(v["num-retries"] === undefined ? null : { numRetries: v["num-retries"] }),
            };
        });

    export type Outbound = {
        "request-id": string;
        RequestBody?: RetriesPostRequestBody$.Outbound | undefined;
        "num-retries"?: number | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, RetriesPostRequest> = z
        .object({
            requestId: z.string(),
            requestBody: z.lazy(() => RetriesPostRequestBody$.outboundSchema).optional(),
            numRetries: z.number().int().optional(),
        })
        .transform((v) => {
            return {
                "request-id": v.requestId,
                ...(v.requestBody === undefined ? null : { RequestBody: v.requestBody }),
                ...(v.numRetries === undefined ? null : { "num-retries": v.numRetries }),
            };
        });
}

/** @internal */
export namespace RetriesPostRetries$ {
    export type Inbound = {
        retries: number;
    };

    export const inboundSchema: z.ZodType<RetriesPostRetries, z.ZodTypeDef, Inbound> = z
        .object({
            retries: z.number().int(),
        })
        .transform((v) => {
            return {
                retries: v.retries,
            };
        });

    export type Outbound = {
        retries: number;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, RetriesPostRetries> = z
        .object({
            retries: z.number().int(),
        })
        .transform((v) => {
            return {
                retries: v.retries,
            };
        });
}

/** @internal */
export namespace RetriesPostResponse$ {
    export type Inbound = {
        ContentType: string;
        StatusCode: number;
        RawResponse: Response;
        retries?: RetriesPostRetries$.Inbound | undefined;
    };

    export const inboundSchema: z.ZodType<RetriesPostResponse, z.ZodTypeDef, Inbound> = z
        .object({
            ContentType: z.string(),
            StatusCode: z.number().int(),
            RawResponse: z.instanceof(Response),
            retries: z.lazy(() => RetriesPostRetries$.inboundSchema).optional(),
        })
        .transform((v) => {
            return {
                contentType: v.ContentType,
                statusCode: v.StatusCode,
                rawResponse: v.RawResponse,
                ...(v.retries === undefined ? null : { retries: v.retries }),
            };
        });

    export type Outbound = {
        ContentType: string;
        StatusCode: number;
        RawResponse: never;
        retries?: RetriesPostRetries$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, RetriesPostResponse> = z
        .object({
            contentType: z.string(),
            statusCode: z.number().int(),
            rawResponse: z.instanceof(Response).transform(() => {
                throw new Error("Response cannot be serialized");
            }),
            retries: z.lazy(() => RetriesPostRetries$.outboundSchema).optional(),
        })
        .transform((v) => {
            return {
                ContentType: v.contentType,
                StatusCode: v.statusCode,
                RawResponse: v.rawResponse,
                ...(v.retries === undefined ? null : { retries: v.retries }),
            };
        });
}
