...
export function selectGlobalServerResponseToJSON(
  selectGlobalServerResponse: SelectGlobalServerResponse,
): string {
  return JSON.stringify(
    SelectGlobalServerResponse$outboundSchema.parse(selectGlobalServerResponse),
  );
}

export function selectGlobalServerResponseFromJSON(
  jsonString: string,
): SafeParseResult<SelectGlobalServerResponse, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SelectGlobalServerResponse$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SelectGlobalServerResponse' from JSON`,
  );
}
