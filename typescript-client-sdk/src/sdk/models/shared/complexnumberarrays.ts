...
> = z.object({
  ...
  decimalStrArray: z.array(z.string().transform(v => new Decimal$(v)))
    .optional(),
});
...
> = z.object({
  ...
  decimalArray: z.array(
    z.union([z.instanceof(Decimal$), z.number()]).transform(v =>
      typeof v === "number" ? v : v.toNumber()
    ),
  ).optional(),
  decimalStrArray: z.array(
...
export function complexNumberArraysToJSON(
  complexNumberArrays: ComplexNumberArrays,
): string {
  return JSON.stringify(
    ComplexNumberArrays$outboundSchema.parse(complexNumberArrays),
  );
}

export function complexNumberArraysFromJSON(
  jsonString: string,
): SafeParseResult<ComplexNumberArrays, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ComplexNumberArrays$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ComplexNumberArrays' from JSON`,
  );
}
