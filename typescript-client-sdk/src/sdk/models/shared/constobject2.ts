...
export function constObject2ToJSON(constObject2: ConstObject2): string {
  return JSON.stringify(ConstObject2$outboundSchema.parse(constObject2));
}

export function constObject2FromJSON(
  jsonString: string,
): SafeParseResult<ConstObject2, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ConstObject2$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ConstObject2' from JSON`,
  );
}
