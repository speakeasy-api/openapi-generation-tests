...
export function exampleCarToJSON(exampleCar: ExampleCar): string {
  return JSON.stringify(ExampleCar$outboundSchema.parse(exampleCar));
}

export function exampleCarFromJSON(
  jsonString: string,
): SafeParseResult<ExampleCar, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ExampleCar$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ExampleCar' from JSON`,
  );
}
