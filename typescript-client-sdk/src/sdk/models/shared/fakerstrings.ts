...
/** @internal */
export const FakerStrings$inboundSchema: z.ZodType<
  FakerStrings,
  z.ZodTypeDef,
  unknown
> = z.object({
...
export function fakerStringsToJSON(fakerStrings: FakerStrings): string {
  return JSON.stringify(FakerStrings$outboundSchema.parse(fakerStrings));
}

export function fakerStringsFromJSON(
  jsonString: string,
): SafeParseResult<FakerStrings, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => FakerStrings$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'FakerStrings' from JSON`,
  );
}
