...
export function nestedEnumArrayToJSON(
  nestedEnumArray: NestedEnumArray,
): string {
  return JSON.stringify(NestedEnumArray$outboundSchema.parse(nestedEnumArray));
}

export function nestedEnumArrayFromJSON(
  jsonString: string,
): SafeParseResult<NestedEnumArray, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => NestedEnumArray$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'NestedEnumArray' from JSON`,
  );
}
