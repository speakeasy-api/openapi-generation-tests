...
export const ObjectWithNullEnums$inboundSchema: z.ZodType<
  ...
  unknown
> = z.object({
  nullEnum: z.literal(null).optional(),
  optNullEnum: z.literal(null).optional(),
});
...
export function objectWithNullEnumsToJSON(
  objectWithNullEnums: ObjectWithNullEnums,
): string {
  return JSON.stringify(
    ObjectWithNullEnums$outboundSchema.parse(objectWithNullEnums),
  );
}

export function objectWithNullEnumsFromJSON(
  jsonString: string,
): SafeParseResult<ObjectWithNullEnums, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ObjectWithNullEnums$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ObjectWithNullEnums' from JSON`,
  );
}
