...
export function refQueryParamObjToJSON(
  refQueryParamObj: RefQueryParamObj,
): string {
  return JSON.stringify(
    RefQueryParamObj$outboundSchema.parse(refQueryParamObj),
  );
}

export function refQueryParamObjFromJSON(
  jsonString: string,
): SafeParseResult<RefQueryParamObj, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => RefQueryParamObj$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'RefQueryParamObj' from JSON`,
  );
}
