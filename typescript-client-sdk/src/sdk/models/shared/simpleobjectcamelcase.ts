...
/** @internal */
export const IntEnumVal$inboundSchema: z.ZodNativeEnum<typeof IntEnumVal> = z
  .nativeEnum(IntEnumVal);
...
> = z.object({
  ...
  bigint_str_val: z.string().transform(v => BigInt(v)).optional(),
  bigint_val: z.number().transform(v => BigInt(v)).optional(),
  bool_opt_val: z.boolean().optional(),
  bool_val: z.boolean(),
  date_time_val: z.string().datetime({ offset: true }).transform(v =>
    new Date(v)
  ),
  date_val: z.string().transform(v => new RFCDate(v)),
  decimal_val: z.number().transform(v => new Decimal$(v)).optional(),
...
> = z.object({
  ...
  bigintStrVal: z.bigint().transform(v => `${v}`).optional(),
  bigintVal: z.bigint().transform(v => Number(v)).optional(),
  ...
  decimalVal: z.union([z.instanceof(Decimal$), z.number()]).transform(v =>
...
export function simpleObjectCamelCaseToJSON(
  simpleObjectCamelCase: SimpleObjectCamelCase,
): string {
  return JSON.stringify(
    SimpleObjectCamelCase$outboundSchema.parse(simpleObjectCamelCase),
  );
}

export function simpleObjectCamelCaseFromJSON(
  jsonString: string,
): SafeParseResult<SimpleObjectCamelCase, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SimpleObjectCamelCase$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SimpleObjectCamelCase' from JSON`,
  );
}
