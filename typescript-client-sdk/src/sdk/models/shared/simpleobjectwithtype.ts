...
export function simpleObjectWithTypeToJSON(
  simpleObjectWithType: SimpleObjectWithType,
): string {
  return JSON.stringify(
    SimpleObjectWithType$outboundSchema.parse(simpleObjectWithType),
  );
}

export function simpleObjectWithTypeFromJSON(
  jsonString: string,
): SafeParseResult<SimpleObjectWithType, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => SimpleObjectWithType$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'SimpleObjectWithType' from JSON`,
  );
}
