...
export function taggedObject1ToJSON(taggedObject1: TaggedObject1): string {
  return JSON.stringify(TaggedObject1$outboundSchema.parse(taggedObject1));
}

export function taggedObject1FromJSON(
  jsonString: string,
): SafeParseResult<TaggedObject1, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => TaggedObject1$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'TaggedObject1' from JSON`,
  );
}
