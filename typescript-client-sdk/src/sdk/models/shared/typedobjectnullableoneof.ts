...
export function typedObjectNullableOneOfToJSON(
  typedObjectNullableOneOf: TypedObjectNullableOneOf,
): string {
  return JSON.stringify(
    TypedObjectNullableOneOf$outboundSchema.parse(typedObjectNullableOneOf),
  );
}

export function typedObjectNullableOneOfFromJSON(
  jsonString: string,
): SafeParseResult<TypedObjectNullableOneOf, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => TypedObjectNullableOneOf$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'TypedObjectNullableOneOf' from JSON`,
  );
}
