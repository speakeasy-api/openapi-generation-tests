...
import { createPageIterator, Decimal as Decimal$, PageIterator, Paginator } from "./types";
...
export class ResponseBodies extends ClientSDK {
    ...
    async responseBodyDecimalStr(
    ...
    ): Promise<operations.ResponseBodyDecimalStrResponse> {
        const input$ = request;
        const headers$ = new Headers();
        headers$.set("x-speakeasy-user-agent", SDK_METADATA.userAgent);
        headers$.set("Content-Type", "application/json");
        ...
        const baseURL$ =
            options?.serverURL ||
            this.templateURLComponent(operations.ResponseBodyDecimalStrServerList[0], {
        ...
        const [result$] = await this.matcher<operations.ResponseBodyDecimalStrResponse>()
            .json(200, operations.ResponseBodyDecimalStrResponse$, { key: "decimal" })
            .fail(["4XX", "5XX"])
            .match(response, { extraFields: responseFields$ });
