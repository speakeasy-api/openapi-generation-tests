...
export class Unions extends ClientSDK {
    ...
    async oneOfOverlappingObjects(
        ...
        options?: RequestOptions
    ): Promise<operations.OneOfOverlappingObjectsResponse> {
        const input$ = request;
        const headers$ = new Headers();
        headers$.set("x-speakeasy-user-agent", SDK_METADATA.userAgent);
        headers$.set("Content-Type", "application/json");
        ...
        const [result$] = await this.matcher<operations.OneOfOverlappingObjectsResponse>()
            .json(200, operations.OneOfOverlappingObjectsResponse$, { key: "res" })
            .fail(["4XX", "5XX"])
            .match(response, { extraFields: responseFields$ });
